...
subplot(1,2,1)
imshow(IB1)
title('Folio en blanco 1')
subplot(1,2,2)
imshow(IB2)
title('Folio en blanco 2')
...
subplot(2,2,3)
title('Análisis horizontal Ampliado')
hold off
plot(mean(RB1C),'r'); hold on
plot(mean(GB1C),'g');
plot(mean(BB1C),'b')
...
subplot(2,2,4)
title('Análisis vertical ampliado')
hold off
plot(mean(RB1C,2),'r'); hold on
plot(mean(GB1C,2),'g');
plot(mean(BB1C,2),'b')
...
% 
% 
% DR = @(RB1C) aR + bR ./ (RB1C - cR);
% DG = @(GB1C) aG + bG ./ (GB1C - cG);
% DB = @(BB1C) aB + bB ./ (BB1C - cB);
% 
% DosisR = DR(RB1C);
% figure(1)
% imagesc(DosisR)
...
desvRmeanHRR = std(juntasmeanHR(:,:,1,:),0,4);
desvGmeanHRG = std(juntasmeanHR(:,:,2,:),0,4);
desvBmeanHRB = std(juntasmeanHR(:,:,3,:),0,4);
...
desvRmeanVRR = std(juntasmeanVR(:,:,1,:),0,4);
desvGmeanVRG = std(juntasmeanVR(:,:,2,:),0,4);
desvBmeanVRB = std(juntasmeanVR(:,:,3,:),0,4);
...
% Variación vertical
juntasmeanVR2 = zeros(size(mean(double(I1R(:,:,1)) / maxBitNumber,2),1),size(mean(double(I1R(:,:,1)) / maxBitNumber,2),2),3,2);
...
max(max(DesviacionPromediosR))
max(max(DesviacionPromediosG))
max(max(DesviacionPromediosB))
